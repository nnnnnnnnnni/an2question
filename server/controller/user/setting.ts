...
import { IUserSchema } from "mongo/models";
import crypto from "crypto";

export const setting = async (ctx: Context) => {
  const { name } = ctx.request.body;
  const user = (ctx.session as Session).user;
  const newUser = (await userModel.findOneAndUpdate({ _id: user._id }, { $set: { name: name } }, { new: true })) as IUserSchema;
...
export const password = async (ctx: Context) => {
  ...
  const user = (ctx.session as Session).user;
  const _user = (await userModel.findOne({ _id: user._id })) as IUserSchema;
